...
class TestRouletteStructureRepo(unittest.TestCase):
    ...
    def test_str(self):
        self.assertEqual(str(self.__roulette_repo), "Tower of Infinity Gauntlet, from Ring 1, with the difficulty Low Insane(8.23), you didn't beat this tower yet, and you need to beat it once to eliminate it.\nCitadel of Green Stuff, from Zone 2, with the difficulty Low-Mid Intense(6.39), you beat this tower once, and you need to beat it 3 times to eliminate it.\nIs This A Tower?, from Time-Lost Plain, with the difficulty High-Peak Challenging(5.81), you have beaten this tower enough so that it can be eliminated from the roulette.\n")
